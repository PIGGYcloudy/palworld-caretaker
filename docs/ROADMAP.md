...
v0.9.0 的開箱、備份整合、Web UI 易用性與安全邊界工作已完成；以下保留
規格對照，方便發布驗收。

### 1. Windows 雙擊開箱（已完成）
- [x] 提供雙擊可運行的 `.bat`（包含環境自檢、啟動服務與自動喚醒瀏覽器管理面板）。

### 2. 首次開服精靈（已完成）
- [x] 伺服器密碼由使用者在 UI 手動輸入，不後台隨機生成。
- [x] 支援在精靈中自由選擇自動備份時間點。
- [x] 明確不提供好友連線卡片（避免 VPN / 虛擬網卡環境誤導）。

### 3. 備份位置與整合（已完成）
- [x] 預設備份路徑直接指向遊戲存檔同級目錄 `SaveGames_Backups/`。
- [x] Web 面板提供「開啟 / 查看備份資料夾」按鈕。

### 4. 好友連線卡片（刻意不納入）
- [x] 明確不納入 v0.9.0。

### 5. 遠端 Web UI／區網 IP 綁定切換（已完成）
- [x] 將「本機／家中區網」切換開關收納於 Web UI 的【進階設定】區塊，不放在主要操作介面，避免一般使用者誤操作。
- [x] 維持本機模式為預設；切換至家中區網時清楚提示僅限可信任 LAN/VPN 使用。

### 6. 遊戲參數調整區塊用詞與預設展開狀態（已完成）
- [x] 預設展開、供一般使用者調整的分區標題固定為【常用參數】。
- [x] 預設摺疊、涵蓋完整設定欄位的分區標題固定為【全部參數】。
- [x] 兩個標題直接顯示於介面，不以技術性名稱替代，初始展開／摺疊狀態符合規格。

### 7. Discord 4 步嚮導（已完成）
- [x] Web 面板提供 4 步簡化設定嚮導，GitHub 提供完整文件。

### 8. 備份紀錄時間化與總佔用空間（已完成）
- [x] 備份紀錄僅顯示直觀時間（今天 04:30、昨天 04:30）與檔案大小。
- [x] 頂部顯示總覽，例如「總共 3 份備份，佔用 6.8 GB」。
