...
備份排程、備份目的地與保留版本數分別由 `BACKUP_TIME`、
`PALWORLD_BACKUP_DIR` 與 `BACKUP_RETENTION_COUNT` 決定。`BACKUP_TIME` 支援
`daily-HH:MM`、`every-Nh`、`every-Nd`（N 為 1–365）與 `off`，例如每 7 小時或每 3 天。
面板與首次設定精靈可直接選擇間隔數字及小時／天。排程使用主機本地時間；小時間隔於整點執行、
天間隔於午夜執行，以 1970-01-01 為共同基準，跨午夜不會重設間隔。錯過的時段不補跑。

自動更新使用獨立的 `UPDATE_TIME`（預設 `off`）；面板可勾選啟用並設定每天檢查時間。
備份與自動更新各有儲存按鈕。單純備份不執行 SteamCMD；更新流程先存檔、備份，
再由 SteamCMD 驗證並套用更新。更新排程即使沒有新版，也會執行這個維護流程。
原本運行的伺服器完成後重新啟動，原本關閉則維持關閉。
Windows 原生模式由管理面板背景程序執行排程，關閉瀏覽器不影響排程；結束面板程序則停止排程。
Linux 使用現有 timer，Docker 使用 supervisor。

伺服器狀態區提供啟動／關閉／重啟、遊戲連接埠及可顯示／複製的遊戲密碼。
密碼預設隱藏，僅在操作時透過已驗證的獨立端點讀取，不包含於一般狀態回應。
管理面板的 `127.0.0.1:8765` 與 REST 管理 API 不等於遊戲監聽位址。
Hamachi 玩家請使用主機的 Hamachi IPv4 位址與遊戲 UDP 連接埠（預設 8211），並允許該連接埠通過 Windows 防火牆。
只有需要讓其他電腦管理面板時，才需要調整面板範圍；若遊戲程序本身被額外設定為只監聽 `127.0.0.1`，遠端玩家就無法連線。
